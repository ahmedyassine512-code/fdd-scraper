...
def find_pdf_link(session, detail_url):
    ...
    for a in soup.find_all("a", href=True):
        href = a["href"]
        # Check the PATH component only, not the raw string -- a link like
        # "franchise_fdd.pdf?id=642040" has query params after the
        # extension, which a naive .endswith(".pdf") on the full href
        # misses entirely.
